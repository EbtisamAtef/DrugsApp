    ...
    func application(_ application: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any])
        -> Bool {
            ...
            GIDSignIn.sharedInstance().handle(
                url, sourceApplication:options[UIApplicationOpenURLOptionsKey.sourceApplication] as? String, annotation: [:])
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        ...
        if let err = error{
            ...
            Auth.auth().signInAndRetrieveData(with: credential) { (authResult, error) in
                if let error = error {
                    print("faild to login with google \(error)")
                    return
                }
                let userID = Auth.auth().currentUser!.uid
                print("you succeed to login with google\(userID)")
                ...
                //rootViewController.pushViewController(, animated: true);
